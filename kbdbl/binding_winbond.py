...
class Usblinux():
    ...
    def sendbrightness(self, brightness):
        #6 is max brightness
        self.write(bytearray.fromhex('06be150001020d000'+str(round(brightness*0.06))+'000000ff00000000000000000000000000000000000000')) #Brightness, 00 - not powered, 07 - blinking
    ...
    def sendhex(self, jsoncolors):
        ...
        for keyname, color in jsoncolors.items():
            scancodes+="%0.2x" % self.keyboard[keyname] #Just convert HEX value to hex string
            colorstr+=color+'00'
            count+=1
            if count==5:
